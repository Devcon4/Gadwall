...
export const parsePassports = (data: string) => {
    ...
    return data
        .split('\n\n')
        .filter(v => v)
        .map(v => JSON.parse(`{${v.replace(/[\w#]+/g, '"$&"').trimEnd().replace(/[\n\s]/g, ',')}}`))
        .filter(v => ValidKeys.every(k => v[k]))
        .length;
}

const ValidColors = ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'];
export const validators: {[k:string]: (k:string,v:{[k:string]:string})=>boolean} = {
...
export const parsePassportsStrict = (data: string) => {
    ...
    return data
        ...
        .filter(v => v)
        .map(v => JSON.parse(`{${v.replace(/[\w#]+/g, '"$&"').trimEnd().replace(/[\n\s]/g, ',')}}`))
        .filter(v => ValidKeys.every(k => validators[k](k, v)))
        .length;
}